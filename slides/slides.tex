...
\begin{document}
  ...
  \begin{frame}
    ...
    \begin{itemize}
      \onslide<1->{
        \item Swalens et al.\ planned to identify composable ``building blocks'' (thread creation, message passing, etc.) that could be used to build common concurrency models.~\footcite{Swalens2014}
      }
      \vfill
      \onslide<2->{
        \item Marr and D'Hondt proposed a unifying abstraction for high-level language virtual machines.~\footcite{Marr2012}
      }
      \vfill
      \onslide<3->{
        \item Ziv et al.\ formalized a theory for safely composing concurrency control.~\footcite{Ziv2015}
      }
    \end{itemize}
  \end{frame}
